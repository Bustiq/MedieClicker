...
func _ready() -> void:
	var better_bread = BuildingUpgrade.new(1, "BREAD", "Cada rollo de cocina da +0.005 medies/s (Es el doble!)", $"../Control/Buildings/".get_child(0), 1, load("res://Sprites/Upgrades/1_Better_bread.png"), 3, 0.005, 1)
	var sesame_seeds = BuildingUpgrade.new(2, "SEEDS", "Cada rollo de cocina da +0.01 medies/s", $"../Control/Buildings/".get_child(0), 3, load("res://Sprites/Upgrades/2_Sesame_seeds.png"), 8, 0.01, 1)
	var thicker_flour = BuildingUpgrade.new(6, "FLOUR", "Cada rollo de cocina da +0.04 medies/s", $"../Control/Buildings/".get_child(0), 11, load("res://Sprites/Upgrades/3_Thicker_flour.png"), 20, 0.04, 1)
	var automatic_stirrer = BuildingUpgrade.new(23, "STIRRER", "Cada rollo de cocina da +0.05 medies/s", $"../Control/Buildings/".get_child(0), 11, load("res://Sprites/Upgrades/4_Automatic_stirer.png"), 35, 0.05, 1)
	
	

	var options_unlock = ShowElement.new(34, "SETTINGS", "¿Querés bajarle el volúmen bro?", 0.5, load("res://Sprites/Upgrades/options.png"), 1.5, $"../Control/Settings")
	var stats = ShowElement.new(36, "STATS", "Te muestra estadísticas de la partida (en opciones)", 70, load("res://Sprites/Upgrades/stats.png"), 100, $"../Control/Settings/PauseBackground/Stats")
	...
	var golazos = PongScoreBonus.new(5, "GOAL", "Cada gol en el pong te da 5 medies más", 15, load("res://Sprites/Upgrades/goal.png"), 5, 5)
	...
	var fosforos = BuildingUpgrade.new(27, "MATCHES", "Cada parrilla te da +4.5 medies/s", $"../Control/Buildings/".get_child(1), 600, load("res://Sprites/Upgrades/fosforos.png"), 3, 4.5)
	var encendedor = BuildingUpgrade.new(28, "LIGHTER", "Cada parrilla te da +7 medies/s", $"../Control/Buildings/".get_child(1), 1000, load("res://Sprites/Upgrades/encendedor.png"), 7, 7)
	var fogata = BuildingUpgrade.new(29, "CAMP", "Cada parrilla te da +10 medies/s", $"../Control/Buildings/".get_child(1), 1700, load("res://Sprites/Upgrades/fogata.png"), 12, 10)
	...
	var watch_ads = SurferDodgeBonus.new(17, "ADS", 10, load("res://Sprites/Upgrades/watch_ads.png"), 25, 0.4)
	var daily_login = SurferDodgeBonus.new(18, "DAILY", 30, load("res://Sprites/Upgrades/daily_login.png"), 50, 0.6)
	var energy_system = SurferDodgeBonus.new(19, "RNERGY", 50, load("res://Sprites/Upgrades/energy_system.png"), 75, 1.2)
