...
func _ready() -> void:
	...
	var automatic_stirrer = BuildingUpgrade.new(6, "Batidora Automática", "Cada rollo de cocina da +0.05 medies/s", $"../Control/Buildings/".get_child(0), 11, load("res://Sprites/Upgrades/4_Automatic_stirer.png.png"), 35, 0.05, 1)
	var pong_unlock = MinigameUnlockUpgrade.new(3, "Desbloquear Pong", "Desbloquea el minijuego Pong (Se controla con la rueda del mouse)", ScoreType.type.PONG, 20, load("res://Sprites/pong_icon.png"), 20, $"../Control/PongViewportContainer/PongViewport", "res://Scenes/Minigames/Pong/pong_game.tscn")
	var tetris_unlock = MinigameUnlockUpgrade.new(4, "Desbloquear Tetris", "Desbloquea el minijuego Tetris (Se controla con los botones del mouse, espacio, flecha hacia arriba y C)", ScoreType.type.TETRIS, 0, load("res://Sprites/Upgrades/tetris.png"), 0, $"../Control/TetrisViewportContainer/TetrisViewport", "res://Scenes/Minigames/Tetris/tetris_game.tscn")
	...
	var raid_4 = ClickUpgrade.new(15, "Raid 4", 128, load("res://Sprites/Upgrades/raid_4.png"), 1500, 0.08)
	...
	var watch_ads = SurferDodgeBonus.new(17, "Mirar anuncios", 10, load("res://Sprites/Upgrades/watch_ads.png"), 25, 0.2)
	var daily_login = SurferDodgeBonus.new(18, "Login diario", 40, load("res://Sprites/Upgrades/daily_login.png"), 75, 0.4)
	var energy_system = SurferDodgeBonus.new(19, "Sistema de energía", 70, load("res://Sprites/Upgrades/energy_system.png"), 150, 0.5)
	...
	var victory = Victory.new(22, "Trofeo", "Gracias por jugar! Agradecimientos especiales a Mateo Fernández por la música", ScoreType.type.OTHER, 3000, load("res://Sprites/Upgrades/medie_clicker.png"), 2000)
